...
One caveat if you type check your own config: these are TypeScript sources and they import `node:path` and read `process.cwd()`, so a `tsc` run that walks into them needs `@types/node`. It is listed as an optional peer for that reason. `skipLibCheck` will not cover it, because these are `.ts` sources rather than `.d.ts` declarations.
...
| `astro`          | `false`                                            | Adds eslint-plugin-astro and astro-eslint-parser for `.astro`, with the TypeScript parser on the frontmatter, the house rules, `dryer/unbroken-sentences`, and the Astro component rules (`component-slot`, `spread-attributes`, `explicit-fragment`, `class-list`, `image-layout`, `no-frontmatter-classes`). |
| `html`           | `false`                                            | Adds @html-eslint/parser for `.html` files and enforces `dryer/no-bare-divs` on HTML markup.                                                                                                                                                                                                                   |
...
| `top-level-functions`     | Functions belong at the top of the file rather than nested inside another function; single-use inline named functions (callbacks, event handlers) stay inline.                                                                               |
...
| `no-bare-divs`            | A `<div>` with no attributes says nothing about what it holds, how it looks, or why it exists. The only place HTML gives a bare div semantic meaning is inside a `<dl>`, where it groups `<dt>` and `<dd>` pairs.                            |
| `no-hidden-classes`       | The Tailwind `hidden` class sets `display: none`, removing content from screen readers as well as the screen. Enforces `sr-only` and `not-sr-only`, or the `hidden` attribute if content must be hidden completely.                          |
| `hidden-attribute`        | The HTML `hidden` attribute removes an element from the accessibility tree, so screen readers will not announce it. A warning.                                                                                                               |
| `heading-group`           | Headings followed by paragraphs belong in an `<hgroup>` rather than a generic wrapper. A warning.                                                                                                                                            |
| `component-slot`          | An Astro component that cannot take children is a page written in the wrong place. Requires a default `<slot />`. Pages under `pages/` are exempt.                                                                                           |
| `spread-attributes`       | An Astro component must rest leftover props as `...attributes` or `...rest` and spread that same name onto an element. Pages under `pages/` are exempt.                                                                                      |
| `explicit-fragment`       | The shorthand `<>` hides from search; Astro components write `<Fragment>` instead.                                                                                                                                                           |
| `class-list`              | A ternary, `\|\|` or `&&` in `class={...}` belongs in Astro's `class:list`. A bare identifier is prop forwarding and is left alone.                                                                                                          |
| `image-layout`            | Astro's `<Image>` and `<Picture>` should carry `inferSize` and a responsive `layout` so Astro generates `srcset` and `sizes`. A warning.                                                                                                     |
| `no-frontmatter-classes`  | A hard-coded class string in the frontmatter belongs on the element. Props and imports are left alone. A warning.                                                                                                                            |
...
It defines the block-axis and shorthand spacing utilities (`mbs`, `mbe`, `mbl`, `mli`, the negative margins and the auto margins), the scroll offsets (`scroll-mbs`, `scroll-mbe`, `scroll-mbl`, `scroll-mli`, `scroll-pbs`, `scroll-pbe`, `scroll-pbl`, `scroll-pli`), the insets and gaps (`inset-bs`, `inset-be`, `inset-bl`, `inset-li` with their `-auto` and `-full` forms, `gap-bl`, `gap-li`), sibling spacing and rules (`space-bs`, `space-is`, `divide-bs`, `divide-is`), inline sizing (`inline-*`, `min-inline-*`, `max-inline-*` with the whole keyword set Tailwind's `w`, `min-w` and `max-w` take), `block-full`, the borders on both axes (`border-bs`, `border-be`, `border-bl`, `border-s`, `border-e`, `border-li`), the block-edge radii (`rounded-bs`, `rounded-be`) and `icon`.

The borders and the sibling rules take a width in steps, an arbitrary length or a colour, the way their physical counterparts do: `border-bs-2`, `border-bs-[0.2rem]`, `border-bs-red-500`, `border-bs-accent` (any `--color-*` theme token), `border-s-2`, `border-li-red-500`, `divide-bs-2` and `divide-is-red-500` all resolve.

**Both axes count in rem.** Tailwind counts a border width in whole pixels, on the block axis and the inline one alike; this file counts it in sixteenths of a rem, so `border-bs-2` and `border-s-2` are both twice the hairline rather than two of somebody else's device pixels, and a border two hairlines across the text is two hairlines down it as well. Tailwind 4 ships `border-bs`, `border-be`, `border-s` and `border-e` itself; this file restates them after, so the rem width is the one that lands while Tailwind's own `border-*-style` still applies. `border-bl`, `border-li` and the whole `divide-bs`/`divide-is` family Tailwind does not ship at all. Every class named here was compiled with the real `@tailwindcss/cli` (4.3.3) and checked against the declarations it emitted.

Every keyword class the rule can recommend is covered by a test, so `mx-auto` is never answered with an `mli-auto` that generates nothing. The widths and colours are covered the same way. The one deliberate gap left is the pixel family: there is no `mbs-px`, because a class that is both logical and a pixel is not an improvement, and `dryer/natural-size` answers `mt-px` by asking for a spacing step instead. A viewport width keeps Tailwind's spelling in the class name (`inline-dvw`, from `w-dvw`) but is measured on the inline axis with `dvi`, which is the axis the text actually runs along.
