...
| `types-directory`         | A shape the application shares belongs where every part of it can find it. A warning.                                                                                                                                                   |
...
It defines the block-axis and shorthand spacing utilities (`mbs`, `mbe`, `mbl`, `mli`, `pbs`, `pbe`, `pbl`, `pli`, and the negative margins), the scroll offsets (`scroll-mbl`, `scroll-mli`, `scroll-pbl`, `scroll-pli`), the insets and gaps (`inset-bs`, `inset-be`, `inset-bl`, `inset-li`, `gap-bl`, `gap-li`), sibling spacing and rules (`space-bs`, `space-is`, `divide-bs`, `divide-is`), inline sizing (`inline-*`, `min-inline-*`, `max-inline-*` and their `-full`, `-auto` and `-fit` forms), `block-full`, the block-axis borders (`border-bs`, `border-be`, `border-bl`) and `icon`.
