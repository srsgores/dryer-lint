...
The block-axis borders and the sibling rules take a width in steps, an arbitrary length or a colour, the way their physical counterparts do: `border-bs-2`, `border-bs-[0.2rem]`, `border-bs-red-500`, `border-bs-accent` (any `--color-*` theme token), `divide-bs-2` and `divide-is-red-500` all resolve. Tailwind counts a border width in whole pixels; these count it in sixteenths of a rem, so `border-bs-2` is twice the hairline rather than two of somebody else's device pixels. Tailwind 4 ships `border-bs` and `border-be` itself, in pixels, and this file restates them after, so the rem width is the one that lands and Tailwind's own `border-block-start-style` still applies; `border-bl` and the whole `divide-bs`/`divide-is` family Tailwind does not ship at all. Each of those six class names was compiled with the real `@tailwindcss/cli` (4.3.3) and checked against the declarations it emitted.
